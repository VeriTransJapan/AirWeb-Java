 ...
 */
public class DoPostActionAW extends HttpServlet {
     ...
     * ログ出力用インスタンス
     */
    private final Logger log = Logger.getLogger(DoPostActionAW.class);
    ...
    public void doPost(final HttpServletRequest request, final HttpServletResponse response) throws IOException,
        ...
        if (key == null) {
            ...
            out.println("ERR");
            return;
        } else if (key.containsKey(Constants.AWRES_M_STATUS)) {
            StringBuffer sb = new StringBuffer();
            Iterator ite = key.keySet().iterator();
            String entry = null;
            while (ite.hasNext()) {
                entry = (String)ite.next();
                sb.append(entry);
                sb.append("=");
                sb.append((String)key.get(entry));
            }
