 ...
 * 全Actionクラスの基底クラス
 */
public abstract class MerchantSampleAction extends Action {

    /**
     * ログ出力用インスタンス
     */
    private static Logger log = Logger.getLogger(MerchantSampleAction.class);
     ...
     */
    protected abstract void addInfomation(CommodityForm form);
     ...
     */
    protected CommodityDetail[] createCommodityDetails(final CommodityForm commForm) {
        ...
        MerchantConf conf = MerchantConf.getInfo();
        CommodityDetail[] commodityDetail = new CommodityDetail[nameLen];

        if (id == null || id.length == 0) {
            id = new String[nameLen];
            for (int i = 0; i < nameLen; i++) {
                id[i] = conf.getDummyCommodityId() + i;
            }
        }
        if (janCode == null || janCode.length == 0) {
            janCode = new String[nameLen];
            for (int i = 0; i < nameLen; i++) {
                janCode[i] = conf.getDummyCommodityJanCode() + i;
            }
        }
    ...
    public ActionForward execute(final ActionMapping mapping, final ActionForm form, final HttpServletRequest request,
        ...
        CommodityForm commForm = (CommodityForm)form;
     ...
     */
    protected abstract EncryptionKey send(String sessionId, String orderId, CommodityForm commForm,
